let encode liste = 
	...
	let rec loop liste acc_tup acc_n = match liste with
		| [] -> acc_tup
		| head::next::tail when head = next -> loop (next :: tail) acc_tup (acc_n + 1)
		| head::tail -> loop tail ( acc_tup @ [(acc_n, head)] ) 1
	in
	loop liste [] 1
...
let rec print_list liste func = match liste with
	| [] -> print_char '\n'
	| head::tail -> (func head) ; print_list tail (func)

(* Test lists *)
let empty = []
let char_list = ['a'; 'a'; 'a'; 'a'; 'b'; 'b'; 'c'; 'd'; 'd']
